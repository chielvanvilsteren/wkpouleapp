...
export default async function RanglijstPage() {
  ...
  if (scoresZichtbaar && userIds.length > 0) {
    ...
    entries.sort((a, b) => {
      if ((b.totaal ?? 0) !== (a.totaal ?? 0))
        return (b.totaal ?? 0) - (a.totaal ?? 0);
    ...
    <div className="max-w-4xl mx-auto px-4 py-10">
                ...
                {entries.map((entry, idx) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3 text-center text-gray-700">
                      {entry.selectie_punten !== null ? (
                        entry.selectie_punten
                      ) : (
                        <span className="text-gray-400">—</span>
                      )}
                    </td>
                    <td className="px-4 py-3 text-center text-gray-700">
                      {entry.basis_xi_punten !== null ? (
                        entry.basis_xi_punten
                      ) : (
                        <span className="text-gray-400">—</span>
                      )}
                    </td>
                    <td className="px-4 py-3 text-center text-gray-700">
                      {entry.incidenten_punten !== null ? (
                        entry.incidenten_punten
                      ) : (
                        <span className="text-gray-400">—</span>
                      )}
                    </td>
                    <td className="px-4 py-3 text-center">
                      {entry.totaal !== null ? (
                        <span className="font-bold text-oranje-600 text-lg">
                          {entry.totaal}
                        </span>
                      ) : (
      ...
      {!scoresZichtbaar && entries.length > 0 && (
        <p className="text-center text-sm text-gray-400 mt-4">
          Scores worden gepubliceerd zodra de organisator de officiële uitslag
          heeft ingevoerd.
        </p>
      )}
    </div>

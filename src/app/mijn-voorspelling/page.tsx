...
export default async function MijnVoorspellingPage() {
  ...
  if (
    (profileRaw as { is_deelnemer: boolean } | null)?.is_deelnemer === false
  ) {
    redirect("/admin");
  }

  const [{ data: predictionRaw }, { data: uitslagRaw }, { data: scoreRaw }] =
    await Promise.all([
      supabase.from("predictions").select("*").eq("user_id", user.id).single(),
      supabase
        .from("master_uitslag")
        .select("inzendingen_open, inzendingen_deadline, scores_zichtbaar")
        .eq("id", 1)
        .single(),
  ...
  const now = new Date();
  const deadline = uitslag?.inzendingen_deadline
    ? new Date(uitslag.inzendingen_deadline)
    : null;

...
const SEG_W = 30, SEG_H = 20, SIZE_W = 30, SIZE_H = 20
const H_WAVE = 0.5, V_WAVE = 0.3, SWING = 0.2, SPEED = 0.5
...
function makeDutchTexture(): THREE.CanvasTexture {
  ...
  const s = 200 / 3
  ctx.fillStyle = '#AE1C28'; ctx.fillRect(0, 0, 300, s)
  ctx.fillStyle = '#FFFFFF'; ctx.fillRect(0, s, 300, s)
  ctx.fillStyle = '#21468B'; ctx.fillRect(0, s * 2, 300, s)
...
export default function WavingFlag({ className = '', flagUrl }: Props) {
  ...
  useEffect(() => {
    ...
    const flagMat = new THREE.MeshLambertMaterial({ map: makeDutchTexture(), side: THREE.DoubleSide })
    ...
    const animate = () => {
      ...
      for (let y = 0; y <= SEG_H; y++)
        for (let x = 0; x <= SEG_W; x++)
          posAttr.setZ(x + y * (SEG_W + 1), Math.sin(H_WAVE * x + V_WAVE * y - t) * SWING * x / 4)

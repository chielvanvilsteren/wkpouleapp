      ...
      <div className="max-w-6xl mx-auto px-4">
        <div className="flex items-center justify-between h-15 py-2.5">
          ...
          <Link href="/" className="flex items-center gap-2.5 group">
            ...
            <div className="leading-tight font-jetbrains">
              <span className="font-black text-white text-base tracking-tight">WK Pool</span>
              <span className="font-black text-oranje-400 text-base tracking-tight"> 2026</span>
            </div>
          </Link>
            ...
            {user ? (
              <div className="flex items-center gap-2 ml-3 pl-3 border-l border-white/15">
                ...
                <div className="flex items-center gap-2 bg-white/10 rounded-full pl-1 pr-3 py-1">
                  ...
                  <span className="text-sm text-white/80 font-medium">{profile?.display_name}</span>
                </div>
                <button
              ...
              <div className="flex items-center gap-2 ml-3">
                <Link href="/login" className="px-3 py-2 rounded-lg text-sm font-medium text-white/70 hover:text-white hover:bg-white/10 transition-colors">
                  Inloggen
                </Link>
                <Link href="/register" className="px-4 py-2 rounded-lg text-sm font-semibold bg-oranje-500 hover:bg-oranje-600 text-white transition-colors">
                  Aanmelden
                </Link>
              </div>
            )}
        ...
        {menuOpen && (
          <div className="md:hidden border-t border-white/10 py-3 space-y-0.5">
            <MobileLink href="/ranglijst" onClick={close}>Ranglijst</MobileLink>

            {user && profile?.is_deelnemer !== false && (
              <>
                <MobileLink href="/mijn-voorspelling" onClick={close}>Pre-Pool</MobileLink>
                <MobileLink href="/wk-poule" onClick={close}>WK Poule</MobileLink>
              </>
            )}
            ...
            {user ? (
              ...
              <div className="pt-3 mt-2 border-t border-white/10 flex gap-2 px-1">
                <Link href="/login" onClick={close} className="flex-1 text-center px-3 py-2 rounded-lg text-sm font-medium text-white/70 hover:bg-white/10 transition-colors">
                  Inloggen
                </Link>
                <Link href="/register" onClick={close} className="flex-1 text-center px-3 py-2 rounded-lg text-sm font-semibold bg-oranje-500 hover:bg-oranje-600 text-white transition-colors">
                  Aanmelden
                </Link>
              </div>
            )}

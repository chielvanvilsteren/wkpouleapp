...
const STAGE_LABELS: Record<string, string> = {
  ...
  qf: 'Kwartfinales', sf: 'Halve finales', '3rd': '3e plaatswedstrijd', final: 'Finale',
}

export default function AdminMatchResults({ matches }: Props) {
  const [results, setResults] = useState<Map<number, { home: number | null; away: number | null; finished: boolean }>>(
    new Map(matches.map((m) => [m.id, { home: m.home_score, away: m.away_score, finished: m.is_finished }]))
  )
  ...
  const toggleStage = (key: string) => {
    setOpenStages((prev) => { const next = new Set(prev); next.has(key) ? next.delete(key) : next.add(key); return next })
  }
  ...
  const handleSave = async () => {
    ...
    for (const [id, r] of Array.from(results.entries())) {
      const { error } = await supabase
        .from('matches')
        .update({ home_score: r.home, away_score: r.away, is_finished: r.finished })
        .eq('id', id)
                ...
                {stageMatches.map((match) => {
                  ...
                  return (
                    <div key={match.id} className="flex items-center gap-2 px-4 py-2">
                      ...
                      <span className="flex-1 text-sm text-right text-gray-700 truncate">{match.home_team}</span>
                      <div className="flex items-center gap-1 shrink-0">
                        <input type="number" min={0} max={99} value={r.home ?? ''} onChange={(e) => setScore(match.id, 'home', e.target.value)} className="w-12 text-center input-field text-sm font-bold px-1 py-1" placeholder="—" />
                        <span className="text-gray-400">—</span>
                        <input type="number" min={0} max={99} value={r.away ?? ''} onChange={(e) => setScore(match.id, 'away', e.target.value)} className="w-12 text-center input-field text-sm font-bold px-1 py-1" placeholder="—" />
                      </div>
                      <span className="flex-1 text-sm text-left text-gray-700 truncate">{match.away_team}</span>
                      <button

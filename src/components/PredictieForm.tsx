  ...
  return (
    <div className="flex items-center gap-3 mb-4">
      <div className="flex-1 bg-gray-200 rounded-full h-2.5">
        <div className="bg-oranje-500 h-2.5 rounded-full transition-all duration-300" style={{ width: `${pct}%` }} />
      </div>
      <span className="text-sm font-semibold text-gray-700 whitespace-nowrap">{filled} / {total}</span>
    </div>
  )
}

function PlayerGrid({ values, onChange, prefix, disabled }: { values: string[]; onChange: (idx: number, val: string) => void; prefix: string; disabled: boolean }) {
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
      {values.map((val, idx) => (
        <div key={idx} className="flex items-center gap-2">
          <span className="text-xs text-gray-400 w-6 text-right shrink-0">{idx + 1}.</span>
          <input type="text" value={val} onChange={(e) => onChange(idx, e.target.value)} disabled={disabled} className="input-field text-sm" placeholder={`${prefix} ${idx + 1}`} />
        </div>
      ))}
    </div>
  )
...
export default function PredictieForm({ initialPrediction, isOpen, score }: Props) {
  const [selectie, setSelectie] = useState<string[]>(toArray(initialPrediction?.selectie, 26))
  const [basisXi, setBasisXi] = useState<string[]>(toArray(initialPrediction?.basis_xi, 11))

  const [isDefinitief, setIsDefinitief] = useState(initialPrediction?.is_definitief ?? false)
  ...
  const handleSave = async (definitief: boolean) => {
    ...
    const { error } = await supabase.from('predictions').upsert(payload, { onConflict: 'user_id' })
    ...
    <div className="space-y-8">
      ...
      {isDefinitief && (
        <div className="bg-green-50 border border-green-300 text-green-800 px-4 py-3 rounded-lg text-sm font-medium">
          ✅ Voorspelling definitief ingezonden. Aanpassingen zijn niet meer mogelijk.
        </div>
      )}
      ...
      <div className="card">
        ...
        <PlayerGrid values={selectie} onChange={updateSelectie} prefix="Speler" disabled={!canEdit} />
      </div>
      ...
      <div className="card">
        ...
        <PlayerGrid values={basisXi} onChange={updateBasisXi} prefix="Speler" disabled={!canEdit} />
      </div>
        ...
        <div className="flex flex-col items-center gap-3">
          {!confirmDefinitief ? (
            ...
            <div className="bg-amber-50 border border-amber-300 rounded-xl p-5 w-full max-w-md text-center">
              ...
              <p className="text-sm text-amber-700 mb-4">Na definitief inzenden kun je je pre-pool voorspelling <strong>niet meer aanpassen</strong>.</p>
